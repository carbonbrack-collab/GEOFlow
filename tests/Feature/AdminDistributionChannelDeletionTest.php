...
class AdminDistributionChannelDeletionTest extends TestCase
{
    ...
    public function test_delete_flow_copy_is_available_in_every_supported_locale(): void
    {
        foreach (['zh_CN', 'en', 'ja', 'es', 'ru', 'pt_BR'] as $locale) {
            ...
            $this->assertNotSame(
                'admin.distribution.delete.heading',
                __('admin.distribution.delete.heading'),
                "Missing channel deletion copy for locale {$locale}."
            );
            $this->assertNotSame(
                'admin.distribution.status.deleting',
                __('admin.distribution.status.deleting'),
                "Missing deleting status copy for locale {$locale}."
            );
            $this->assertNotSame(
                'admin.distribution.delete.operation_wait',
                __('admin.distribution.delete.operation_wait'),
