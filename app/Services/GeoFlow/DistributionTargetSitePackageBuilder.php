...
HTACCESS;
    ...
    {
        return TargetThemeRegistry::bodyClass((string) ($settings['active_theme'] ?? ''));
    }
...
CSS;
        ...
        $themes = TargetThemeRegistry::css();

        return $themes === '' ? $base : $base."\n".$themes."\n";
    }
...
function themeClass(array $settings): string
{
    ...
    foreach ($map as $needle => $class) {
        if (is_string($needle) && is_string($class) && $needle !== '' && str_contains($theme, $needle)) {
            return $class;
        }
    }
